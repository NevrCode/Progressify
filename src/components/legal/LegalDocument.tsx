  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.background }}>
      <ScrollView contentContainerStyle={{ padding: 24, gap: 18 }}>
        <TouchableOpacity accessibilityRole="button" accessibilityLabel="Go back" hitSlop={8} onPress={() => router.canGoBack() ? router.back() : router.replace("/login")}>
          <Text style={{ color: theme.primary, fontWeight: "700" }}>Back</Text>
        </TouchableOpacity>
        <View style={{ gap: 6 }}>
          <Text accessibilityRole="header" style={{ color: theme.textBlack, fontSize: 30, fontWeight: "800" }}>{title}</Text>
          <Text style={{ color: theme.textLight }}>Effective {effectiveDate}</Text>
        </View>
        {sections.map((section) => (
          <View key={section.heading} style={{ gap: 6 }}>
            <Text accessibilityRole="header" style={{ color: theme.text, fontSize: 18, fontWeight: "800" }}>{section.heading}</Text>
