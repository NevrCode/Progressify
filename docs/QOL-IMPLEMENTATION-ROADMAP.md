...
- **Wave 5 / measurement preferences: implementation complete; migration deployment pending.** Account-level metric/imperial preference is owner-scoped. Workout and body-measurement input/display use centralized conversion while API/database values remain canonical kilograms and centimetres. Nutrition macros and food quantity units remain unchanged.
...
- **Wave 5 / favorites and recents: implementation complete; migration deployment pending.** Food and exercise search present Favorites, Recent, then Matches with deterministic deduplication. Favorites are owner-scoped and idempotent; recents derive only from actual food logs and exercise sessions.
