...
- **Wave 2 / repeat last workout: complete.** Completion writes an immutable versioned snapshot, and repeating creates a fresh owner-scoped active session plus incomplete local drafts without consulting mutable routine/program state.
- **Wave 2 / rest experience: complete with configurable vibration.** Per-exercise defaults and active/paused timer restoration are implemented. Device-local vibration is configurable and guarded exactly once. Audio remains unavailable because `expo-audio` is not installed.
...
- **Wave 4 / routine structure: complete and migration verified.** One revisioned whole-program layout endpoint atomically owns routine order, exercise order, and explicit superset groups. Expo provides bounded vertical drag gestures plus visible move controls, reload-and-repeat conflict handling, draft-v4 launch snapshots, grouped active-workout rendering, and deterministic post-round rest. A polished finger-following list remains optional device-level enhancement work because no drag-list dependency is installed.
- **Wave 5 / measurement preferences: complete and migration verified.** Account-level metric/imperial preference is owner-scoped. Workout and body-measurement input/display use centralized conversion while API/database values remain canonical kilograms and centimetres. Nutrition macros and food quantity units remain unchanged.
- **Wave 5 / native workout-history date: complete.** Manual date text entry is replaced by a reusable native date-only field with deterministic Android/iOS cancellation and local-calendar `YYYY-MM-DD` serialization.
- **Wave 5 / favorites and recents: complete and migration verified.** Food and exercise search present Favorites, Recent, then Matches with deterministic deduplication. Favorites are owner-scoped and idempotent; recents derive only from actual food logs and exercise sessions.
- **Wave 6 / sync details and conflicts: complete.** The existing owner-scoped SQLite queue now exposes redacted operation details, stable failure categories, strict ordering, individual retry/discard, persisted semantic conflicts, fresh-key resolution support, and query invalidation after replay.
- **Wave 6 / durable deletion undo: complete.** Food entries, saved exercise sessions, and safely deletable empty routines restore the same identity. Pending offline deletes can be cancelled exactly; draft-set undo remains local.
- **Wave 6 / completed snapshots and repeat: implementation complete; migration deployment pending.** Workout start captures immutable launch layout, completion atomically writes ordered final sets, and repeat creates a fresh active session even after source rows are soft-deleted.
